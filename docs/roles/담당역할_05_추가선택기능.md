...
logger = Logger(experiment_name="feature_text2sql")
...
logger = Logger(experiment_name="feature_evaluation")
...
if __name__ == "__main__":
    ...
    logger.write(json.dumps(result, indent=2, ensure_ascii=False))
...
**사용 방법**:
1. Logger 인스턴스 생성
   - experiment_name 형식: `feature_text2sql`, `feature_evaluation`
   - 예: `logger = Logger(experiment_name="feature_text2sql")`

2. 로그 기록
   - `logger.write()` 사용 (print() 대신)
   - 예: `logger.write(f"SQL 쿼리: {sql_query}")`
...
6. **11_데이터베이스_설계.md** - DB 스키마 (Text-to-SQL용)
