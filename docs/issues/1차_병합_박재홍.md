...
### 병합 진행 상황
```
현재 상태: MERGE IN PROGRESS (병합 중)
충돌 파일: requirements.txt
```

### 충돌 파일
- ❌ **requirements.txt**: 양쪽 모두 수정 (BOTH MODIFIED)
...
### 1단계: requirements.txt 충돌 해결
```bash
# upstream 버전 채택
git checkout --theirs requirements.txt

# 선택적으로 HEAD의 주석 스타일 반영 (수동 편집)
# 버전 고정이 필요한 패키지는 수동으로 버전 지정
...
- 임베딩 및 Vector DB 적재 모듈 추가
- PostgreSQL + pgvector 초기 설정 스크립트
...
```bash
# README에 다음 내용 추가:
# 1. scripts/ 폴더 파일 역할 설명
# 2. 실험 폴더 명명 규칙 (과도기적 차이 명시)
# 3. 팀원별 담당 모듈 정리
```
