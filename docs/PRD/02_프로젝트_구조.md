...
#### 주요 파일
1. `rag_search.py`: 논문 데이터베이스 검색
2. `web_search.py`: TavilySearchResults 활용 웹 검색
3. `glossary.py`: 용어집 검색 및 설명
4. `summarize.py`: load_summarize_chain 활용 요약
5. `file_save.py`: 대화/요약 내용 파일 저장
...
graph TB
    ...
    subgraph AgentProcessing["🔹 Agent 처리"]
        direction TB
    ...
    subgraph Response["🔶 답변 생성"]
        direction LR
        VectorDB --> LLM[🤖 ChatOpenAI<br/>GPT-4]
        PostgreSQL --> LLM
        TavilyAPI --> LLM
    ...
    style Response fill:#fff3e0,stroke:#e65100,stroke-width:3px,color:#000
